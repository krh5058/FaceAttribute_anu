function [frame] = javaui(blockvals,condlist)
% javaui.m
% 9/13/13
% Author: Ken Hwang
...
rb2Panel = JPanel(GridLayout(length(condlist),1));
t2 = BorderFactory.createTitledBorder('Task:');
rb2Panel.setBorder(t2);
group1 = ButtonGroup();

for i = 1:length(condlist)
    temprb = JRadioButton(condlist{i});
    temprb.setActionCommand(condlist{i});
    if i == 1
...
left.setMinimumSize(Dimension(150,425));
left.setPreferredSize(Dimension(150,425));
left.add(tf1Panel);
left.add(rb2Panel);
...
t3 = BorderFactory.createTitledBorder('Blocks:');
...
frame.setVisible(1);
    ...
    function onConfirm(obj,evt) % When confirm button is pressed
        ...
        cond = selectedModel1.getActionCommand();
        ...
        elseif any(cellfun(@isempty,listout)) % Check for empty entries in block list
            javax.swing.JOptionPane.showMessageDialog(frame,'There are unused blocks!','Block list check',javax.swing.JOptionPane.INFORMATION_MESSAGE);
        else
            ...
            if result==javax.swing.JOptionPane.YES_OPTION
                ...
                setappdata(frame,'UserData',{char(sid),cond,listout});
                frame.dispose();
            else
            end
        end
